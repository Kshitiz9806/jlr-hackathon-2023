...
static vsomeip::service_t service_id = 0x1112;
...
public:
    ...
    hello_world_client() :
                    ...
                    app_(rtm_->create_application("client", "../../Client/local.json"))
    {
    }
    ...
    void on_message_cbk(const std::shared_ptr<vsomeip::message> &_response)
    {
                ...
                && vsomeip::return_code_e::E_OK == _response->get_return_code())
        {
            ...
            std::string steer = "none";
            if(stoi(location_)>stoi(resp)){
                steer = "left";
            }
            else if (stoi(location_) < stoi(resp))
            {
                steer = "right";
            }
            location_ = resp;
            writeFile(stoi(location_), steer);
            // stop();
        }
    }
...
private:
    ...
    std::string location_ = "0";

    void writeFile(int left, std::string steer) {
        std::string pref = "[CLIENT]";
        std::string path = "../../jlr-hackathon-2023/src/Components/logs_data/log_client_dummy_1.log";
        std::ofstream fp;
        fp.open(path, std::ios_base::app);
        fp << "\n" << pref << " Moving to: " << left << " "<<steer ;
        fp.close();
    }
};
