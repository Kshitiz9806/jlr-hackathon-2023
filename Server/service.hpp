...
public:
    ...
    hello_world_service() :
                    ...
                    app_(rtm_->create_application("service", "../../Server/local.json")),
                    stop_(false),
    ...
    bool init()
    {
        ...
        if (!app_->init()) {
            ...
            std::string path = "../../jlr-hackathon-2023/src/Components/logs_data/log_server_dummy_1.log";
            std::ofstream fp;
            fp.open(path, std::ios_base::app);
            fp << "[SERVER] [ERROR] Could not initialise the server!!!\n";
            fp.close();
            return false;
        }
    ...
    void on_message_cbk(const std::shared_ptr<vsomeip::message> &_request)
    {
        ...
        // vsomeip::byte_t* data = _request->get_payload()->get_data();
        // LOG_INF("Data output %s", data);
